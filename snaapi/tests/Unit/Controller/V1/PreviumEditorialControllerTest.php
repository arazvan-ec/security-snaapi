...
use App\Controller\V1\PreviumEditorialController;
...
#[CoversClass(PreviumEditorialController::class)]
class PreviumEditorialControllerTest extends TestCase
{
    private OrchestratorChain&MockObject $orchestratorChain;
    private PreviumEditorialController $controller;

    protected function setUp(): void
    {
        $this->orchestratorChain = $this->createMock(OrchestratorChain::class);
        $this->controller = new PreviumEditorialController($this->orchestratorChain);
    ...
    public function getEditorialByIdMustReturnEditorial(): void
    {
        ...
        $this->orchestratorChain
            ->expects($this->once())
            ->method('handler')
            ->with('previum', $this->isInstanceOf(Request::class))
